...
$sql = "SELECT * FROM contact where Status='Pendente' AND solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM contact where Status='Pendente' AND solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC"); 
...
echo "<a href=../listacontact/listagemusuario_pend.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }
elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listacontact/listagemusuario_pend.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
elseif (($pagina > 1) and ($pagina < $total_paginas)) {
echo "<a href=../listacontact/listagemusuario_pend.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";      
echo "<a href=../listacontact/listagemusuario_pend.php?pag=".($pagina+1)."$url_filtros>Próximo</a>";
