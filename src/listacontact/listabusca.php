...
$sql = "SELECT * FROM contact where motivo LIKE '%$busca%' OR mensagem LIKE '%$busca%' OR solicitante LIKE '%$busca%' ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados, sem limitação de resultados, para a paginação

$consulta = mysql_query("SELECT * FROM contact where motivo LIKE '%$busca%' OR mensagem LIKE '%$busca%' OR solicitante LIKE '%$busca%' ORDER BY ticket DESC"); // Seleciona o campo ticket da nossa tabela contact
...
echo "<a href=../listacontact/listabusca.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listacontact/listabusca.php?busca=$busca&pag=".($pagina+1).">Próximo</a>"; 
        }
