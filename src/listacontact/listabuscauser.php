...
$sql = "SELECT * FROM contact where (solicitante='$usuariologado') AND ((motivo LIKE '%$busca%') OR (mensagem LIKE '%$busca%')) ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados, sem limitação de resultados, para a paginação

$consulta = mysql_query("SELECT * FROM contact where (solicitante='$usuariologado') AND ((motivo LIKE '%$busca%') OR (mensagem LIKE '%$busca%')) ORDER BY ticket DESC"); 
...
if ($linha['status'] == 'Atendido') {  // Linhas na cor verde com status Atendido
echo '<tr class="success">';
echo '<td>'.$linha['data'].'</td>';
echo '<td>'.$linha['motivo'].'</td>';
echo '<td>'.$linha['ticket'].'</td>';
echo '<td>'.$linha['status'].'</td>';
echo '<td><a href="../listacontact/visualizar.php?ticket='.$ticket.'">Visualizar</a></td>';
echo '</tr>';
}
...
echo "<a href=../listacontact/listabuscauser.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listacontact/listabuscauser.php?busca=$busca&pag=".($pagina+1).">Próximo</a>"; 
        }
