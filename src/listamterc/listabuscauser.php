...
$sql = "SELECT * FROM cadmater where (solicitante='$usuariologado') AND ((codfab LIKE '%$busca%') OR (descricao LIKE '%$busca%') OR (fab LIKE '%$busca%') OR (comentario LIKE '%$busca%')) ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// consulta no BD sem limite
$buscar = "SELECT * FROM cadmater where (solicitante='$usuariologado') AND ((codfab LIKE '%$busca%') OR (descricao LIKE '%$busca%') OR (fab LIKE '%$busca%') OR (comentario LIKE '%$busca%')) ORDER BY ticket DESC";
...
echo "<a href=../listamterc/listabuscauser.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listamterc/listabuscauser.php?busca=$busca&pag=".($pagina+1).">Próximo</a>"; 
        }
