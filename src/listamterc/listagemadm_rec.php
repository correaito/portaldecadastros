...
$sql = "SELECT * FROM cadmater where Status='Recusado'  $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadmater where Status='Recusado'  $filtro_sql ORDER BY ticket DESC"); // Seleciona tudo da nossa tabela cadmater
...
echo "<a href=../listamterc/listagemadm_rec.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listamterc/listagemadm_rec.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
