...
<?php
include("../config.php");
include("../restrito.php"); // conexão com o BD
session_start();
...
$sql = "SELECT * FROM cadmater  WHERE 1=1 $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado
$consulta = mysql_query("SELECT * FROM cadmater  WHERE 1=1 $filtro_sql ORDER BY ticket DESC"); // Seleciona tudo da nossa tabela cadmater
...
<div class="section">
  ...
  <p><i class="icon-list-alt icon-white"></i> Solicitações de material de terceiro</p>
  <div class="cadform">
...
<table class="table table-hover table-condensed table-custom" data-row-style="rowStyle" style="width: 100% !important; margin: 0 !important;">
<thead>
<tr>
<th>Data</th>
  <th>Usuario</th>
...
{
$ticket=$linha['ticket'];
$protol=$linha['descricao'];
$protol = '<div class="truncate-text" title="'.htmlspecialchars($protol, ENT_QUOTES, 'UTF-8').'">'.$protol.'</div>';
$statusquo=$linha['status'];
...
echo '<td><a href="../listamterc/alteraradm.php?ticket='.$ticket.'">Editar</a></td>'; 
echo '</tr>';
}
...
echo '<td>'.$linha['ticket'].'</td>';
echo '<td>'.$linha['status'].'</td>';
echo '<td><a href="../listamterc/atendendo.php?ticket='.$ticket.'">Editar</a></td>';
echo '</tr>';
}
...
if ($linha['atendente'] == $_SESSION['login'] || $_SESSION['nivel'] == 1) {
    echo '<td><a href="../listamterc/alteraradm.php?ticket='.$ticket.'">Continuar</a></td>';
} else {
...
echo "<a href=../listamterc/listagemadm.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listamterc/listagemadm.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
