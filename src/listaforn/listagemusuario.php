...
$sql = "SELECT * FROM cadforn where solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadforn where solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC"); 
...
if ($linha['status'] == 'Cadastrado') { // Linhas na cor verde com status Atendido
...
echo "<a href=../listaforn/listagemusuario.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }
elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaforn/listagemusuario.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
elseif (($pagina > 1) and ($pagina < $total_paginas)) {
echo "<a href=../listaforn/listagemusuario.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";      
echo "<a href=../listaforn/listagemusuario.php?pag=".($pagina+1)."$url_filtros>Próximo</a>";
