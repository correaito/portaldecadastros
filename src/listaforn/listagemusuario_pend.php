...
$sql = "SELECT * FROM cadforn where Status='Pendente' AND solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadforn where Status='Pendente' AND solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC"); 
...
echo '<td><a href="../listaforn/visualizar.php?ticket='.$ticket.'">Visualizar</a></td>';
echo '</tr>';
}
...
echo "<a href=../listaforn/listagemusuario_pend.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaforn/listagemusuario_pend.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
