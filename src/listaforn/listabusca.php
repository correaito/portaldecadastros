...
<?php
if ($_GET['a'] == 'erro'){
/* Gera a mensagem de erro para o usuario informando que ele não tem permissão de alteração da solicitação */
?>
...
$sql = "SELECT * FROM cadforn where razao LIKE '%$busca%' OR cnpj LIKE '%$busca%' OR cidade LIKE '%$busca%' OR comentario LIKE '%$busca%' ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadforn where razao LIKE '%$busca%' OR cnpj LIKE '%$busca%' OR cidade LIKE '%$busca%' OR comentario LIKE '%$busca%' ORDER BY ticket DESC"); 
...
echo "<a href=../listaforn/listabusca.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";    
        }
elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaforn/listabusca.php?busca=$busca&pag=".($pagina+1).">Próximo</a>"; 
        }
elseif (($pagina > 1) and ($pagina < $total_paginas)) {
echo "<a href=../listaforn/listabusca.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";      
echo "<a href=../listaforn/listabusca.php?busca=$busca&pag=".($pagina+1).">Próximo</a>";
