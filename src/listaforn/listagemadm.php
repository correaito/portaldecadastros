...
$sql = "SELECT * FROM cadforn  WHERE 1=1 $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadforn  WHERE 1=1 $filtro_sql ORDER BY ticket DESC"); 
...
<div class="section">
  ...
  <p><i class="icon-list-alt icon-white"></i> Solicitações de Fornecedor</p>
  <div class="cadform">
...
echo '<td><a href="../listaforn/alteraradm.php?ticket='.$ticket.'">Editar</a></td>';
echo '</tr>';
}
...
echo '<td>'.$linha['ticket'].'</td>';
echo '<td>'.$linha['status'].'</td>';
echo '<td><a href="../listaforn/atendendo.php?ticket='.$ticket.'">Editar</a></td>';
echo '</tr>';
}
...
if ($linha['atendente'] == $_SESSION['login'] || $_SESSION['nivel'] == 1) {
    echo '<td><a href="../listaforn/alteraradm.php?ticket='.$ticket.'">Continuar</a></td>';
} else {
...
echo "<a href=../listaforn/listagemadm.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }
elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaforn/listagemadm.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
elseif (($pagina > 1) and ($pagina < $total_paginas)) {
echo "<a href=../listaforn/listagemadm.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";      
echo "<a href=../listaforn/listagemadm.php?pag=".($pagina+1)."$url_filtros>Próximo</a>";
