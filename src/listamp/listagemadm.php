...
$sql = "SELECT * FROM cadmatp  WHERE 1=1 $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadmatp  WHERE 1=1 $filtro_sql ORDER BY ticket DESC"); 
...
<div class="section">
  ...
  <p><i class="icon-list-alt icon-white"></i> Solicitações de material próprio</p>
  <div class="cadform">
...
<tr class="filters">
  ...
  <th style="padding: 5px !important;"><input type="text" name="f_descricao" value="<?php echo isset($_GET['f_descricao']) ? htmlspecialchars($_GET['f_descricao']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
  <th style="padding: 5px !important;"><input type="text" name="f_unimed" value="<?php echo isset($_GET['f_unimed']) ? htmlspecialchars($_GET['f_unimed']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
  <th style="padding: 5px !important;"><input type="text" name="f_codfab" value="<?php echo isset($_GET['f_codfab']) ? htmlspecialchars($_GET['f_codfab']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
  <th style="padding: 5px !important;"><input type="text" name="f_fab" value="<?php echo isset($_GET['f_fab']) ? htmlspecialchars($_GET['f_fab']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
  <th style="padding: 5px !important;"><input type="text" name="f_centros" value="<?php echo isset($_GET['f_centros']) ? htmlspecialchars($_GET['f_centros']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
  ...
  <th style="padding: 5px !important; text-align: center;"><button type="submit" class="btn btn-mini btn-info" style="margin-top: 0px;"><i class="icon-search icon-white" style="margin-top: -1px; vertical-align: middle;"></i></button></th>
</tr>
</thead>
<tbody>
...
{
$ticket=$linha['ticket'];
$descricaoFull = $linha['descricao']; // A string do banco já é UTF-8. Não usar utf8_encode para não duplicar!
$protol = '<div class="truncate-text" title="'.htmlspecialchars($descricaoFull, ENT_QUOTES, 'UTF-8').'">'.$descricaoFull.'</div>';

if ($linha['Status'] == 'Cadastrado') { // Linhas na cor verde com status Cadastrado
echo '<tr class="success">';
echo '<td>'.$linha['data'].'</td>';
echo '<td>'.$linha['solicitante'].'</td>';
echo '<td>'.$protol.'</td>';
echo '<td>'.$linha['unimed'].'</td>';
echo '<td>'.$linha['codfab'].'</td>';
echo '<td>'.$linha['fab'].'</td>';
echo '<td>'.$linha['centros'].'</td>';
echo '<td>'.$linha['ticket'].'</td>';
echo '<td>'.$linha['Status'].'</td>';
echo '<td><a href="../listamp/alteraradm.php?ticket='.$ticket.'">Editar</a></td>';
echo '</tr>';
}
...
echo '<td>'.$linha['ticket'].'</td>';
echo '<td>'.$linha['Status'].'</td>';
echo '<td><a href="../listamp/atendendo.php?ticket='.$ticket.'">Editar</a></td>';
echo '</tr>';
}
...
echo '<td>'.$linha['ticket'].'</td>';
echo '<td>'.$linha['Status'].'</td>';
if ($linha['atendente'] == $_SESSION['login'] || $_SESSION['nivel'] == 1) {
    echo '<td><a href="../listamp/alteraradm.php?ticket='.$ticket.'">Continuar</a></td>';
} else {
    echo '<td><span style="color:#999; cursor:help;" title="Em atendimento por ' . $linha['atendente'] . '"><i class="icon-lock"></i></span></td>';
}
echo '</tr>';
}
...
echo "<a href=../listamp/listagemadm.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listamp/listagemadm.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
...
<div id="alterado" class="modal hide fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
  <div class="modal-header">
    <button type="button" class="close" data-dismiss="modal" aria-hidden="true">×</button>
    <h3 id="myModalLabel">Concluído!</h3>
  </div>
