...
$sql = "SELECT * FROM cadmatp where (solicitante='$usuariologado') AND ((codfab LIKE '%$busca%') OR (descricao LIKE '%$busca%') OR (fab LIKE '%$busca%') OR (avaliacao LIKE '%$busca%') OR (ncm LIKE '%$busca%') OR (comentario LIKE '%$busca%')) ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT ticket FROM cadmatp where (solicitante='$usuariologado') AND ((codfab LIKE '%$busca%') OR (descricao LIKE '%$busca%') OR (fab LIKE '%$busca%') OR (avaliacao LIKE '%$busca%') OR (ncm LIKE '%$busca%') OR (comentario LIKE '%$busca%')) ORDER BY ticket DESC");
...
echo "<a href=../listamp/listabuscauser.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listamp/listabuscauser.php?busca=$busca&pag=".($pagina+1).">Próximo</a>"; 
        }
