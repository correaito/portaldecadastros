...
$sql = "SELECT * FROM cadmatp where solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadmatp where solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC"); 
...
<tr class="filters">
  ...
  <th style="padding: 5px !important;"><input type="text" name="f_ncm" value="<?php echo isset($_GET['f_ncm']) ? htmlspecialchars($_GET['f_ncm']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
...
echo "<a href=../listamp/listagemusuario.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listamp/listagemusuario.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
