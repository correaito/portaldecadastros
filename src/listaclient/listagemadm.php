...
$sql = "SELECT * FROM cadclient  WHERE 1=1 $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados, sem limitação de resultados, para a paginação

$consulta = mysql_query("SELECT * FROM cadclient  WHERE 1=1 $filtro_sql ORDER BY ticket DESC"); 
...
<div class="section">
  ...
  <p><i class="icon-list-alt icon-white"></i> Solicitações de Cliente</p>
  <div class="cadform">
...
<!-- Início da grade de solicitações de cliente -->
...
<tr class="filters">
  ...
  <th style="padding: 5px !important;"><input type="text" name="f_razao" value="<?php echo isset($_GET['f_razao']) ? htmlspecialchars($_GET['f_razao']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
  <th style="padding: 5px !important;"><input type="text" name="f_cnpj" value="<?php echo isset($_GET['f_cnpj']) ? htmlspecialchars($_GET['f_cnpj']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
  <th style="padding: 5px !important;"><input type="text" name="f_cidade" value="<?php echo isset($_GET['f_cidade']) ? htmlspecialchars($_GET['f_cidade']) : ''; ?>" style="width: 90%; margin: 0; padding: 2px 5px; font-size: 11px; font-weight: normal; border-radius: 4px; border: 1px solid #ccc; background: #fff;" placeholder="Filtrar..."></th>
...
echo '<td><a href="../listaclient/alteraradm.php?ticket='.$ticket.'">Editar</a></td>';
echo '</tr>';
}
...
echo '<td>'.$linha['ticket'].'</td>';
echo '<td>'.$linha['status'].'</td>';
echo '<td><a href="../listaclient/atendendo.php?ticket='.$ticket.'">Editar</a></td>';
echo '</tr>';
}
...
if ($linha['atendente'] == $_SESSION['login'] || $_SESSION['nivel'] == 1) {
    echo '<td><a href="../listaclient/alteraradm.php?ticket='.$ticket.'">Continuar</a></td>';
} else {
...
echo "<a href=../listaclient/listagemadm.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaclient/listagemadm.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
