...
$sql = "SELECT * FROM cadclient where razao LIKE '%$busca%' OR cnpj LIKE '%$busca%' OR cidade LIKE '%$busca%' OR comentario LIKE '%$busca%' ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados, sem limitação de resultados, para a paginação

$consulta = mysql_query("SELECT * FROM cadclient where razao LIKE '%$busca%' OR cnpj LIKE '%$busca%' OR cidade LIKE '%$busca%' OR comentario LIKE '%$busca%' ORDER BY ticket DESC"); 
...
echo "<a href=../listaclient/listabusca.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaclient/listabusca.php?busca=$busca&pag=".($pagina+1).">Próximo</a>"; 
        }
