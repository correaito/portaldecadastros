...
$sql = "SELECT * FROM cadclient where Status='Recusado' AND solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados, sem limitação de resultados, para a paginação

$consulta = mysql_query("SELECT * FROM cadclient where Status='Recusado' AND solicitante='$usuariologado'  $filtro_sql ORDER BY ticket DESC"); 
...
echo "<a href=../listaclient/listagemusuario_rec.php?pag=".($pagina-1)."$url_filtros>Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaclient/listagemusuario_rec.php?pag=".($pagina+1)."$url_filtros>Próximo</a>"; 
        }
