...
$sql = "SELECT * FROM cadservp where (solicitante='$usuariologado') AND ((descricao LIKE '%$busca%') OR (comentario LIKE '%$busca%')) ORDER BY ticket DESC LIMIT $inicio,$limite";

$resultado = mysql_query($sql);

// Banco de dados da paginação, ilimitado

$consulta = mysql_query("SELECT * FROM cadservp where (solicitante='$usuariologado') AND ((descricao LIKE '%$busca%') OR (comentario LIKE '%$busca%')) ORDER BY ticket DESC"); 
...
echo "<a href=../listaservpres/listabuscauser.php?busca=$busca&pag=".($pagina-1).">Anterior</a>";    
        }

elseif (($pagina == 1) and ($pagina < $total_paginas))
{
echo "<a href=../listaservpres/listabuscauser.php?busca=$busca&pag=".($pagina+1).">Próximo</a>"; 
        }
